...
namespace Aeroport_Application
{
    public partial class Boarding : Form
    {
        ...
        private void SaveEtatButton_Click(object sender, EventArgs e)
        {
            ...
            pta.UpdateBoardingByIDPassager("BO", DateTime.Now, idPassager);
               /* dt = pta.GetDataByIDVol(volID);
                PassagerDataGridView.Enabled = true;
                PassagerDataGridView.DataSource = dt;
                Pour ne pas enlever le search, letat fafiche changeraa et au reload prochain tout ira bien */

                PassagerDataGridView.SelectedRows[0].Cells["Etat"].Value = "En bord";
                UpdateStatistics();
        ...
        protected void UpdateStatistics()
        {
            ...
            int nbnci = Convert.ToInt32(pta.CountPassengerByEtatAndVolID("BO", volID));
            PourcentagePassagerLabel.Text = "Passagers en bord: " + (nbnci).ToString() + "/" + nbtotal.ToString();
        }
        private void PassaagerSelectedChanged(object sender, EventArgs e)
        {
            //BoardingCheckBox.Checked = false;
            ...
            if (PassagerDataGridView.SelectedRows.Count > 0)
            {
                ...
                if (etat == "BO") DateBoardingLabel.Text = dt.Rows[0]["Date_Boarding_In_Passager"].ToString();
            }
        }
